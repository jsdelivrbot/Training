...
class Store {
    ...
    static addBook(book){
        // first get books with static method
        const books = Store.getBooks();
        console.log(books);
        // push book passed to the books array
        books.push(book);
        // set it in local storage, don't forget to tstringify the array.
        localStorage.setItem('books', JSON.stringify(books));
    }
    static removeBook(isbn){
        const books = Store.getBooks();
        console.log(isbn);
        // remove matching isbn
...
document.getElementById('book-form').addEventListener('submit', function(event){
    ...
    if(title === '' || author === '' || isbn === ''){
        ...
        ui.addBookToList(book);
        // add to Local Storage - call static method
        Store.addBook(book);
